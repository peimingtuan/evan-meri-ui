...
import { PaginationType } from './Props';
import { isEvnetNum, warn } from '../../utils/tools';

export const usePagination = (props: PaginationType, emit: any) => {
  /**
   * 分页器包含的组件：
   ...
   */
  const setCurrentPage = (type: string, pageNum: number = 1, isEmit: boolean = true) => {
   ...
   * @param {Number} v
   */
  const setCurrentPageSize = (v: number) => {
  ...
  watchEffect(() => {
    // 根据传入的 total 计算总页数
    totalPagesNum.value = Math.ceil((props.total as number) / currentPageSize.value);
  ...
  const pagerComponents = computed(() => {
    return {
      bind: {
        ...
        repeatClick: Boolean(props.repeatClick)
      },
      event: 'click',
      eventName: setCurrentPage
    };
...
export const usePaginationValidator = () => {
   ...
   * @returns Boolean
   */
  const pagerCountValidator = (v: Number | String) => {
    const num = Number(v);
   ...
   * @returns Boolean
   */
  const layoutValidator = (v: string) => v.split(',').length !== 0;
